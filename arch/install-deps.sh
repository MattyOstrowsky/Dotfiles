#!/usr/bin/env bash
#
# Install-Deps — Interactive dependency installer for Dotfiles (Arch Linux)
#
# Uses pacman for official packages and paru for AUR packages.
# Automatically installs paru if missing (requires base-devel + git).
#
# Usage:
...
# ─── Auto-install paru if missing ────────────────────────────────────────────
ensure_paru() {
  if command -v paru &>/dev/null; then return 0; fi
  echo -e "  ${YELLOW}paru not found — installing AUR helper...${NC}"
  sudo pacman -S --needed --noconfirm base-devel git 2>&1 | tail -3
  local tmpdir
  tmpdir=$(mktemp -d)
  git clone --depth 1 https://aur.archlinux.org/paru-bin.git "$tmpdir/paru-bin" 2>&1 | tail -1
  (cd "$tmpdir/paru-bin" && makepkg -si --noconfirm 2>&1 | tail -5)
  rm -rf "$tmpdir"
...
sys_pkg() {
  ...
  echo "  Running: sudo pacman -S --needed --noconfirm $pkg"
  sudo pacman -S --needed --noconfirm "$pkg" 2>&1 | tail -5
  return "${PIPESTATUS[0]}"
}
...
install_python() {
  ...
  sudo pacman -S --needed --noconfirm python python-pip 2>&1 | tail -1
}
# pip3 comes with python package, but make sure
install_pip3() {
  if exists pip3; then
    echo "  $(pip3 --version 2>/dev/null)"
    return 0
  fi
  sudo pacman -S --needed --noconfirm python-pip 2>&1 | tail -1
}
...
install_trivy() {
  ...
  ensure_paru
  echo "  Running: paru -S --needed --noconfirm trivy"
  paru -S --needed --noconfirm trivy 2>&1 | tail -5
  return "${PIPESTATUS[0]}"
}
...
tool "System"     "unzip"     "unzip"     install_unzip    ""    "pacman"
tool "System"     "fish"      "fish"      install_fish     ""    "pacman"
tool "System"     "fzf"       "fzf"       install_fzf      ""    "pacman"
tool "System"     "bat"       "bat"       install_bat      ""    "pacman"
tool "System"     "ripgrep"   "rg"        install_ripgrep  ""    "pacman"
tool "System"     "fd-find"   "fd"        install_fdfind   ""    "pacman"
tool "System"     "tree"      "tree"      install_tree     ""    "pacman"
tool "System"     "htop"      "htop"      install_htop     ""    "pacman"
...
tool "Runtimes"   "pip3"      "pip3"      install_pip3     ""    "pacman"
...
tool "IaC"       "terraform" "terraform" install_terraform ""   "pacman"
tool "IaC"       "terragrunt" "terragrunt" install_terragrunt "" "binary"
tool "IaC"       "ansible"   "ansible"   install_ansible  ""    "pacman"
